...
class  FieldDate extends Field {
    ...
    */
    static function create($name, $_formTypeId, $type="date", $category=false)
    {
       //call the parent create
       return parent::create($name, $_formTypeId, "date", false);
    }
    ...
    */
    static function delete($name)
    {
        ...
        //delte the values
        FModel::$db->delete("field_date", "_field_id = " . $id, false);
    ...
    */
    static function save($fieldId , $value, $label="N/A"){  
        ...
        FModel::$db->insert("field_date", array("value" => $value, "_field_id" => $fieldId, "label" => $label));
        return FModel::$db->lastInsertId("field_date_id_seq");
    }
    
    
    static function getValue($fieldValueId){
        $data = FModel::$db->select("select value, label from field_date where id = :id",
    ...
    static function deleteValue($id){
        FModel::$db->delete("field_date", "id = ". $id);
    }
    
    static function update($valueId, $data, $label = 'N/A'){
        FModel::$db->update("field_date", array(
            "value"=>$data,
            "label" => $label
        ), "id = $valueId");
    }
